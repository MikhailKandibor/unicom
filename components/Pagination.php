    ...
    public function get()
    {
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1, '<i class="fa fa-long-arrow-left"></i>') . $links;
            # Если текущая страница не первая
            if ($this->current_page < $this->amount)
            # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, '<i class="fa fa-long-arrow-right"></i>');
        }
        $html .= $links . '</ul>';
        # Возвращаем html
        return $html;
